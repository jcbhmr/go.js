...
await rm("codereview.cfg", { force: true });
await rm("CONTRIBUTING.md", { force: true });
await rm("go.env", { force: true });
await rm("LICENSE", { force: true });
await rm("PATENTS", { force: true });
await rm("README.md", { force: true });
await rm("SECURITY.md", { force: true });
await rm("VERSION", { force: true });
...
for await (const entry of glob(`pkg/tool/${goos}_${goarch}/*`)) {
  await chmod(entry, 0o777);
}
